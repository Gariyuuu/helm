...
export async function getCompaniesForUser(userId: string) {
  return safeQuery(
    () => db.query.companies.findMany({ where: eq(companies.userId, userId), orderBy: (c, { asc }) => [asc(c.name)] }),
    []
  );
}
...
export async function getContactsForUser(userId: string) {
  return safeQuery(
    () => db.query.contacts.findMany({ where: eq(contacts.userId, userId), orderBy: (c, { asc }) => [asc(c.name)] }),
    []
  );
}
